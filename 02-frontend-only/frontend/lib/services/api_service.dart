...
/// 🔧 FRONTEND-ONLY MODE: Uses MockApiService instead of real HTTP requests
/// This allows you to learn Flutter UI development without backend complexity
/// 
/// Note: _useMockData is set to true for frontend-only learning
/// When you're ready for backend integration, check out step 03-backend-frontend-connected
class ApiService {
  ...
  // Set to true for frontend-only development with mock data
  // This bypasses all HTTP calls and uses local mock data instead
  ...
  Future<List<Intern>> getAllInterns({
    ...
    if (_useMockData) {
      // Use mock data for frontend-only development
      final mockResponse = await MockApiService.getAllInterns(
        batch: batch,
        search: search,
        limit: limit,
        offset: offset,
      ...
      if (mockResponse['success'] == true && mockResponse['data'] is List) {
        final internsData = mockResponse['data'] as List;
        return internsData.map((json) => _parseInternJson(json)).toList();
      } else {
        throw Exception(mockResponse['error'] ?? 'Unable to load interns');
      }
    }
  ...
  Future<Intern> getIntern(String internId) async {
    if (_useMockData) {
      final mockResponse = await MockApiService.getIntern(internId);
      if (mockResponse['success'] == true && mockResponse['data'] != null) {
        return _parseInternJson(mockResponse['data']);
      } else {
        throw Exception(mockResponse['error'] ?? 'Failed to fetch intern');
      }
    }
  ...
  Future<Intern> createIntern(Intern intern) async {
    if (_useMockData) {
      final mockResponse = await MockApiService.createIntern(intern.toCreateJson());
      if (mockResponse['success'] == true && mockResponse['data'] != null) {
        return _parseInternJson(mockResponse['data']);
      } else {
        throw Exception(mockResponse['error'] ?? 'Failed to create intern');
      }
    }
  ...
  Future<Intern> updateIntern(String internId, Intern intern) async {
    if (_useMockData) {
      ...
      final mockResponse = await MockApiService.updateIntern(internId, updateData);
      if (mockResponse['success'] == true && mockResponse['data'] != null) {
        return _parseInternJson(mockResponse['data']);
      } else {
        throw Exception(mockResponse['error'] ?? 'Failed to update intern');
      }
    }
  ...
  Future<void> deleteIntern(String internId) async {
    if (_useMockData) {
      final mockResponse = await MockApiService.deleteIntern(internId);
      if (mockResponse['success'] != true) {
        throw Exception(mockResponse['error'] ?? 'Failed to delete intern');
      }
      return;
    }
  ...
  Future<int> getInternCount() async {
    if (_useMockData) {
      final mockResponse = await MockApiService.getInternCount();
      if (mockResponse['success'] == true && mockResponse['count'] != null) {
        return mockResponse['count'] as int;
      } else {
        throw Exception(mockResponse['error'] ?? 'Failed to fetch intern count');
      }
    }
  ...
  Future<Map<String, int>> getTaskSummary() async {
    if (_useMockData) {
      final mockResponse = await MockApiService.getTaskSummary();
      if (mockResponse['success'] == true && mockResponse['data'] != null) {
        final data = mockResponse['data'] as Map<String, dynamic>;
        final statusBreakdown = data['statusBreakdown'] as Map<String, dynamic>;
        return statusBreakdown.map((key, value) => MapEntry(key, value as int));
      } else {
        throw Exception(mockResponse['error'] ?? 'Failed to fetch task summary');
      }
    }
